...
export default class QuanLyCapPhepNuocMatTramBom extends React.Component {
    ...
    componentDidMount(){
        ...
        trackPromise(axios
            .get(configData.API_URL + "/quan-ly-cap-phep/nuoc-mat/nha-may-nuoc/thong-tin-ban-do-cong-trinh", {
    ...
    fetch = (params = {}, filter) => {
        ...
        axios
            .get(configData.API_URL + "/quan-ly-cap-phep/nuoc-mat/nha-may-nuoc/loc-giay-phep/"+filter, {
                headers: {'Authorization': 'Bearer ' + getToken()}
            })
            .then((response) => {
                if(response.status === 200)
            ...
            },
            {
                ...
                render: (text, record) => (
                    <div className="d-flex align-items-center justify-content-center">
                        <Link to={"/quan-ly-cap-phep/nuoc-mat/nha-may-nuoc/chinh-sua/"+record.id} title="Chỉnh Sửa"><EditOutlined /></Link>
                            ...
                            <MapContainer className="col-12 h-100 w-100" whenCreated={ mapInstance => { this.mapRef.current = mapInstance } } center={this.state.center} zoom={this.state.zoom}>
                                ...
                                {this.state.dataSource.map((marker, key) => (
                                    ...
                                    <div>
                                        ...
                                        <Link to={'/quan-ly-cap-phep/nuoc-mat/nha-may-nuoc/xem-thong-tin-chung/'+marker.id} className="card-link d-block text-center">Chi tiết công trình</Link>
                                    </div>
                                    </Popup>
                                </Marker> : ""
                                ))}
