...
export default class QuanLyCapPhepKhoanNDD extends React.Component {
    constructor(props)
    {
        super(props)
        this.state = {
    ...
    fetch = (params = {}, filter) => {
        ...
        axios
            .get(configData.API_URL + "/quan-ly-cap-phep/nuoc-duoi-dat/khoan/loc-giay-phep/"+filter, {
                headers: {'Authorization': 'Bearer ' + getToken()}
            })
            .then((response) => {
                if(response.status === 200)
    ...
    render(){        
        const columns = [
              ...
              render: (text, record, index) => (
                <>
                    ...
                    <Modal className="modal-view-file-pdf" bodyStyle={{backgroundColor : '#323639'}} title={record.gp_sogiayphep} width={1000} footer={null} id={record.gp_sogiayphep} visible={this.state.activeModal === index} onCancel={this.hideModal}>
                        <div>
                            {record.tailieu_khoan[0] && record.tailieu_khoan[0] !== undefined ?
                            <iframe width="100%" title="file giấy phép" src={"http://tainguyennuocsonla.s3-ap-southeast-1.amazonaws.com/"+record.tailieu_khoan[0].tailieu_loaigiayphep+"/"+record.tailieu_khoan[0].tailieu_nam+"/"+record.tailieu_khoan[0].tailieu_giayphep}></iframe>
                            : "Không có tài liệu"
                            }
                        </div>
                    </Modal>
            ...
            {
                title: 'Tên giấy phép',
                dataIndex: 'gp_tengiayphep',
                key: 'gp_tengiayphep',
                width: '30%',
                sorter: (a, b) => a.gp_tengiayphep.localeCompare(b.gp_tengiayphep)
            },
            {
              title: 'Ngày ký',
              dataIndex: 'gp_ngayky',
            ...
            },
            {
                ...
                render: (text, record) => (
                    <div className="d-flex align-items-center justify-content-center">
                        <Link to={"/quan-ly-cap-phep/nuoc-duoi-dat/hanh-nghe-khoan/chinh-sua/"+record.id} title="Chỉnh Sửa"><EditOutlined /></Link>
        ...
        return(
			<div className="p-0">
                <Header headTitle="QUẢN LÝ CẤP PHÉP HÀNH NGHỀ KHOAN NƯỚC DƯỚI ĐẤT" previousLink="/quan-ly-cap-phep" showHeadImage={true} layout48={true} />
