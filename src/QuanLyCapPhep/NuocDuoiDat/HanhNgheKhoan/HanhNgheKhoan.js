...
export default class QuanLyCapPhepNuocDuoiDatHanhNgheKhoan extends React.Component {
    ...
    componentDidMount(){
        document.title = "Khoan nước dưới đất";
    ...
    render(){
        ...
        return(
			<div className="p-0">
                <Header headTitle="QUẢN LÝ CẤP PHÉP KHOAN NƯỚC DƯỚI ĐẤT" previousLink="/quan-ly-cap-phep" showHeadImage={true} layout48={true} />
                                ...
                                {this.state.DataGPKTSDNuocDuoiDat.map((marker, key) => (
                                    <Marker position={[marker.hang_muc_ct[0].longitude, marker.hang_muc_ct[0].latitude]} key={key} >
                                        ...
                                        <div>
                                            ...
                                            <Link to={'/quan-ly-cap-phep/nuoc-duoi-dat/hanh-nghe-khoan/xem-thong-tin-chung/'+marker.id} className="card-link d-block text-center">Chi tiết công trình</Link>
                                        </div>
                                        </Popup>
                                    </Marker>
                                ))}
                                        ...
                                        <tbody>
                                            {this.state.DataGPKTSDNuocDuoiDat.map((e, i) => {
                                                return (
                                                    ...
                                                    <td className="text-start align-middle text-nowrap"><div><Link className="text-primary" title="Xem GP" to={'/quan-ly-cap-phep/nuoc-duoi-dat/hanh-nghe-khoan/xem-thong-tin-chung/'+e.id}><EyeOutlined /></Link>&nbsp; &nbsp;<Link to="/quan-ly-cap-phep/nuoc-duoi-dat/hanh-nghe-khoan/cap-moi" title="Sửa"><EditOutlined /></Link>&nbsp; &nbsp;<span title="Xóa" className="text-danger"><DeleteOutlined /></span></div></td>
                                                    <>
