...
const user = getUser();
export default class QuanLyCapPhepQuanLyCapMoiKhoanNDD extends React.Component {
    constructor(props)
    {
        super(props)
        this.state = {
    ...
    handlerDestroyLicense = (id_gp) =>{
        trackPromise(
            axios.get(configData.API_URL + "/quan-ly-cap-phep/nuoc-duoi-dat/khoan/xoa-giay-phep/"+id_gp, {
    ...
    fetch = (params = {}, filter) => {
        ...
        axios
            .get(configData.API_URL + "/quan-ly-cap-phep/nuoc-duoi-dat/tham-do/danh-sach-cap-moi-giay-phep-ktndd/"+user.id+"/"+filter, {
                headers: {'Authorization': 'Bearer ' + getToken()}
            })
            .then((response) => {
                if(response.status === 200)
    ...
    render(){
                ...
                render: (text, record, i) => (
                    <>
                        <div className="d-flex">
                            <Link className="btn text-primary" to={"/quan-ly-cap-phep/nuoc-duoi-dat/hanh-nghe-khoan/theo-doi/"+record.id} title="Theo Dõi"><EyeOutlined /></Link>
                            <Link className="btn text-primary" to={"/quan-ly-cap-phep/nuoc-duoi-dat/hanh-nghe-khoan/chinh-sua/"+record.id} title="Chỉnh Sửa"><EditOutlined /></Link>
        ...
        return(
			<div className="p-0">
                <Header headTitle="QUẢN LÝ CẤP MỚI GIẤY PHÉP HÀNH NGHỀ KHOAN NƯỚC DƯỚI ĐẤT" previousLink="/quan-ly-cap-phep/nuoc-duoi-dat/hanh-nghe-khoan" showHeadImage={true} layoutfull={true} />
